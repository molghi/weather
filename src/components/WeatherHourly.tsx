...
const WeatherHourly = () => {
    ...
    return (
            ...
            <div className="mb-[30px]">
                ...
                <div className="flex items-stretch justify-center gap-[30px]">
                    ...
                    {Array.from({ length: 6 }, (_, i) => nowIndexHourly + 1 + i).map((x, i) => (
                        <div
                            key={i}
                            className="flex flex-col flex-[1_1_16.666667%] transition-all duration-300 p-[10px] rounded-[10px] leading-[1] shadow-[inset_0_0_1px_white] hover:bg-white/10 hover:scale-110 transition-transform"
                        >
                            <div className="flex items-center gap-[10px] mb-[5px] leading-[1]">
                                ...
                                <span>
                                    <svg
                                        ...
                                        viewBox="0 0 512 512"
                                        className="max-w-[16px] w-full fill-white"
                                    >
                                        &lt;
                                ...
                                {/* TIME */}
                                {new Date(weather?.hourly.time[x]).getHours() +
                                    ":" +
                                    new Date(weather?.hourly.time[x]).getMinutes().toString().padStart(2, "0")}
                            </div>

                            {/* AIR TEMPERATURE */}
                            <div
                                className="flex items-center gap-[10px] mb-[5px] cursor-pointer"
                                onClick={() => console.log(`perform C/F conversion`)}
                            >
                                {Math.round(weather?.hourly.temperature_2m[x])}°C{" "}
                                <span title="Feels like" className="opacity-50">
                                    ({Math.round(weather?.hourly.apparent_temperature[x])}°C)
                                </span>
                            </div>
       ...
       [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2]"
                                title={interpretWeathercode(weather?.hourly.weather_code[x])}
                            >
                                {interpretWeathercode(weather?.hourly.weather_code[x])}
                            </div>
                            ...
                            <div className="mt-auto max-h-[20px] flex items-center gap-[10px] mb-[5px]" title="Precipitation">
                                ...
                                <span title="Precipitation probability">{weather?.hourly.precipitation_probability[x]}%</span>
                            </div>
                            ...
                            <div className="flex items-center gap-[15px]">
                                ...
                                <div className="flex items-center gap-[10px]" title="Humidity">
                                    ...
                                    {weather?.hourly.relative_humidity_2m[x]}%
                                </div>
